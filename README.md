...
> ## ⚠️ 首次必读：先校准选择器
>
> 登录态下的页面选择器（搜索结果、岗位详情、打招呼按钮、聊天输入框）是**预设值，尚未在真实登录态下验证过**。BOSS 是单页应用，DOM 会随版本变化——**不校准直接用，很可能解析为空或打招呼失败**（工具会主动给 ⚠️ 提示）。
>
> 第一次使用请务必走一遍：`boss login` 扫码 → `boss search "关键词" --show` 看弹窗能否解析出岗位 → 不行就照 **[选择器校准](#选择器校准首次使用务必做)** 一节改 [`src/config.js`](src/config.js) 的 `SELECTORS`。
